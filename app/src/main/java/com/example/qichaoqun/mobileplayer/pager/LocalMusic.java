...
import com.example.qichaoqun.mobileplayer.adapter.LocalMusicListAdapter;
import com.example.qichaoqun.mobileplayer.base.BasePager;
import com.example.qichaoqun.mobileplayer.bean.VideoBean;

import java.util.ArrayList;

public class LocalMusic extends BasePager implements AdapterView.OnItemClickListener {

    public static final int FLAG = 10;
    private ListView mListView;
    private TextView mTextView;
    private ProgressBar mProgressBar;
    private ArrayList<VideoBean> musicList = null;
        ...
        public void handleMessage(Message msg) {
            ...
            if(msg.what == FLAG){
                ...
                if(musicList != null && musicList.size() > 0){
                    //有数据，进行数据和view适配
                    LocalMusicListAdapter localMusicListAdapter = new LocalMusicListAdapter(context,musicList);
                    mListView.setAdapter(localMusicListAdapter);
    ...
    public View inintView() {
        View view = View.inflate(context, R.layout.local_music_layout,null);
        mListView = view.findViewById(R.id.local_music_list);
        //设置list view的监听事件
        mListView.setOnItemClickListener(this);
        mTextView = view.findViewById(R.id.local_music_text);
        mProgressBar = view.findViewById(R.id.local_music_progress);
        return view;
    }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        //将路径传入并且去，调用自己写的播放器
        Intent intent = new Intent(context,MyMusicPlayerActivity.class);
